...
async function createAdmin() {
  try {
    ...
    console.log("=== CREATING ADMIN (kabhishek76683@gmail.com) ===");
    
    // Delete existing if any
    await Employee.deleteMany({ role: "admin" });
    ...
    console.log("✅ ADMIN CREATED: kabhishek76683@gmail.com / admin@123");

    // 2. Create Staff (different from admin)
    console.log("\n=== CREATING STAFF ===");
    await Employee.deleteMany({ role: "teller" });
    ...
    console.log("✅ STAFF CREATED: staff@horizonbank.com / staff@123");
    ...
    const staffList = await Employee.find({ role: { $ne: "admin" } });
    console.log(`Admins: ${admins.length}`, admins.map(a => a.email).join(", "));
    console.log(`Staff: ${staffList.length}`, staffList.map(s => s.email).join(", "));
